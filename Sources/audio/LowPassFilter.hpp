...
: 	public FilterCommon<_Float, LowPassFilter<_Float, _Order>>
{
protected:
	using base = FilterCommon<_Float, LowPassFilter<_Float, _Order>>;
	friend base;
	using base::base;
	using base::_pi;
	using base::_0;
	...
	static auto init(_Filter& base, int fc, int fs, _Float Q = _Float(1.0))
	{
		...
		else if constexpr (_Order == 2)
		{
			...
			const auto d = _1 / Q;
			const auto b = _05*(_1 - d*_05*std::sin(w)) / (_1 + d*_05*std::sin(w));
